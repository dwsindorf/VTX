...
void PlantMgr::render(){
	...
	TNLeaf::collect_mode = !test5;

	if(update_needed){
		cout<<"clear leafs shadow_mode="<<shadow_mode<<endl;
		TNLeaf::free();
		clearStats();
		TNLeaf::sorted=false;
	}
	...
	if(TNLeaf::collect_mode && TNLeaf::leafs.size>0){
		if(!TNLeaf::sorted){
			TNLeaf::leafs.sort();
			TNLeaf::sorted=true;
		}
        if(!shadow_mode)
        	setProgram();
		//cout<<"render "<<TNLeaf::leafs.size<<" "<<update_needed<<endl;
		TNLeaf::render();
	}
