...
int UniverseModel::getPrototype(int type,char *tmp)
{
	...
	case TN_TEXTURE:
		...
		break;
	case TN_SPRITE:
		sprintf(tmp,"Sprite(\"firs2x2\",FLIP,1,1e-6,1,1,1,0,0,0)\n");
		break;
	case TN_PLANT:
		{
		char bstr1[256];
	...
	case TN_CRATERS:
		...
		break;
	case TN_ROCKS3D:
		sprintf(tmp,"rocks3d(1,1e-05,0.8,0.163,0.1,0.1,0,noise(GRADIENT|NLOD,0,2))[Color(0.5,0.5,0.5)]\n");
		break;
	case TN_ROCKS:
		sprintf(tmp,"rocks(1,1e-05,0.8,0.163,0.1,0.1,0,noise(GRADIENT|NLOD,0,2))[Color(0.5,0.5,0.5)]\n");
		break;
	case TN_FOG:
		sprintf(tmp,"fog(0.0)\n");
		break;
...
ModelSym* UniverseModel::getObjectSymbol(int type){
	...
	case TN_ROCKS3D:
		return new ModelSym("Rocks3D",type);
	case TN_ROCKS:
