...
//	}
	...
	double bias=0;
	double f=1;
	double d1=0.3+0.2*s[6];
	double l1=0.3+0.1*s[7];
	double d2=0.3+0.3*s[8];
	double l2=0.3+0.3*s[9];
	if(terrain_type!=GN_ICY && use_theme>0.95){
		...
		case GN_OCEANIC:
			...
			g1=Color(0.0,0.1,0.0);
			g2=Color(0.3,0.2,0.0);
			bias=0.1+0.8*r[6];
			sat=0.6+0.1*s[4];
			val=0.8+0.3*s[5];
			mix=Color(0.1,0.2,0);
...
//		mix=Color(0.5+2*r[7],0.5*r[8],0.2+0.3*r[9]);
		colors[0]=mix;
		colors[1]=tc.darken(f*d1);
		colors[2]=tc.lighten(f*l2);
		colors[3]=tc;
		colors[4]=tc.darken(f*d2);
		colors[5]=tc.lighten(f*l2);
		if(bias){
			//double f=0;
			double delta=2.0/6;
			for(int i=0;i<6;i++){
				if(i<3)
					colors[i]=colors[i].mix(g1,bias*(1.0-i*(delta)));
				else
					colors[i]=colors[i].mix(g2,bias*(i*(delta)));
			}
		}
	}
	for(int i=0;i<ncolors;i++){
...
std::string Planetoid::randFeature(int type) {
	...
	case RND_OCEAN_EXPR:
		...
		break;
	case RND_CRATERS:
		sprintf(buff,"craters(ID%d,%d,%g,%g,%1.2f,%1.2f,%1.2f,%1.2f,%1.2f,%1.2f,0.8,0.6,0.05,0,0,%1.2f,%1.2f)",
		(int)(5+0.5*s[1]), // id
		...
		(Ampl),            // depth
		(0.3+0.3*r[6]),    // impact
		Noise*(0.9+0.3*s[7]),    // vnoise
		Noise*(0.9+0.3*s[7]),    // rnoise
		(0.2+0.1*s[9]),    // rise
		(Ampl),            // drop
	...
	case RND_HROUNDED:
		...
		break;
	case RND_VOLCANOS:
		 sprintf(buff,"craters(ID%d,%d,%g,%g,%1.2f,%1.2f,%1.2f,%1.2f,%1.2f,%1.2f,%1.2f,%1.2f,%1.2f,0.05,0,%1.2f,%1.2f)",
		(int)(5+0.25*s[2]), // rand id
		(int)(8+0.5*s[4]), // levels
		(Maxs),            // max size
		(0.3+0.2*s[3]),    // delta size
		(0.1+0.6*Prob),    // probability
		(Ampl),            // depth
		(0.2+0.1*s[6]),    // impact
		Noise*(0.9+0.3*s[7]),    // noise
		Noise*(0.9+0.3*s[7]),    // noise
		(Ampl),            // rise
		(0.1*r[9]),        // drop
		(0.2+0.2*s[10]),   // rim
	...
	case RND_HCRATERS:
		...
		break;
	case RND_SURFACE_CRATERS:
		Prob=r[12]; // probability
		Ampl=0.1+r[13]; // ampl/rise/drop
		Maxs=2e-6*(1+0.5*r[14]); // max size
		str+=randFeature(RND_CRATERS);
		break;
	case RND_SURFACE_VOLCANOS:
		Prob=r[12]; // probability
		Ampl=6+r[13]; // ampl/rise/drop
		Maxs=2e-6*(1+0.5*r[14]); // max size
	...
	case RND_GLOBAL_NOISE:
		...
		//str+=std::to_string(7+s[8]); // orders
		str+=",7,";
		//str+=std::to_string(0.5+s[8]); // homogeneity
		str+="0.5,0.29,2.5";
	...
	case RND_SURFACE_BANDS:
		...
		0.01*s[7],         // phi bias
		0.01*s[8],         // ht bias
		0.05*(1+0.25*s[9]), // bmp bias
		0.05*(1+0.25*s[10])  // slope bias
		);
		str=buff;
...
static std::string newErodeTex(Planetoid *planet){
	std::string str;
	if(TheScene->use_tmps==NO_TMPS){
		char buff[2048];
...
static std::string newSurfaceBands(Planetoid *planet){
	...
	return Planetoid::randFeature(RND_SURFACE_BANDS);
}

static std::string newDualGlobalTex(Planetoid *planet){
	char buff[2048];
...
static std::string rndSurfaceNoise(double f){
	if(f>0.75)
		return Planetoid::randFeature(RND_SURFACE_RIDGES);
	else if(f>0.5)
...
std::string Planetoid::newSurfaceDetail(Planetoid *planet){
	...
	case GN_VOLCANIC:
		...
		break;
	case GN_CRATERED:
		Ampl=2+0.5*r[13]; // ampl/rise/drop
