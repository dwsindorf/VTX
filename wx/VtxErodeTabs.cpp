...
EVT_CHECKBOX(ID_INV,  VtxErodeTabs::OnChangeEvent)
...
SET_SLIDER_EVENTS(FILL_CHANNELS,VtxErodeTabs, FillChannels)
SET_SLIDER_EVENTS(FILL_DRAMPL,  VtxErodeTabs, FillDrAmpl)
SET_SLIDER_EVENTS(FILL_DRAINMIX,VtxErodeTabs, FillDrainMix)
SET_SLIDER_EVENTS(FILL_ORDERS,  VtxErodeTabs, FillOrders)
SET_SLIDER_EVENTS(FILL_FREQ,    VtxErodeTabs, FillFreq)
SET_SLIDER_EVENTS(FILL_FALLOFF, VtxErodeTabs, FillFalloff)
...
void VtxErodeTabs::getObjAttributes()
{
	...
	//       [4]channels [5]ampl(drain) [6]mix(erode) [7]orders [8]delf [9]atten
	int idx = 0;
	TNode *a;
	auto get = [&](ExprSliderCtrl *s, double def) {
		a = args[idx++];
		if (a) s->setValue(a); else s->setValue(def);
	};
	...
	get(FillDrAmplSlider,    0.5);   // drainage amplitude
	get(FillDrainMixSlider,  1.0);   // erosion amplitude
...
void VtxErodeTabs::setObjAttributes()
{
	...
	wxString s = "erode(";
	wxString n = "";
	if (m_inv->GetValue()) { if(n.IsEmpty()) n+="NEG"; else n+="|INV"; }
	if (m_sqr->GetValue()) n += n.IsEmpty() ? "SQR" : "|SQR";
	if (m_ss->GetValue())  n += n.IsEmpty() ? "SS" : "|SS";
	if (!n.IsEmpty()) s += n + ",";

	s += FillAmplSlider->getText()      + ","; // depth      [0]
	s += FillLevelSlider->getText()     + ","; // start      [1]
	s += FillMarginSlider->getText()    + ","; // range      [2]
	s += FillTransportSlider->getText() + ","; // power      [3]
	s += FillChannelsSlider->getText()  + ","; // channels   [4]
	s += FillDrAmplSlider->getText()    + ","; // drain ampl [5]
	s += FillDrainMixSlider->getText()  + ","; // erode ampl [6]
	s += FillOrdersSlider->getText()    + ","; // orders     [7]
	s += FillFreqSlider->getText()      + ","; // delf       [8]
	s += FillFalloffSlider->getText();         // atten      [9]
	s += ")";
